...
* Usage: opengrads -lbc "downscale_forcing_0.01deg.gs [fctl] [time1] [time2] [lon1] [lon2] [lat1] [lat2] [resdlo] [outpath]"
*
* author: Ming Pan
...
lat1=subwrd(args, 6)
lat2=subwrd(args, 7)
resdlo=subwrd(args, 8)
outpath=subwrd(args, 9)
...
*   For precipitation, simple interpolation
    'define pcplo=ave(apcpsfc, t='t', t='t+step-1')'
...
*   9) Calculate longwave from emission ratio and Stephan-Boltzman emission at high resolution
    ...
    'define tmplo=ave(tmp2m, t='t', t='t+step-1')'
    'define tmpls=tmplo-demlo*('lapse')'
    
    'define prslo=ave(pressfc, t='t', t='t+step-1')'
    'define prsls=prslo*exp('g/ra'*demlo/((tmplo+tmpls)/2))'
    
    'define sfhlo=ave(spfh2m, t='t', t='t+step-1')'
...
*        say hh
        if (hh=0)
            'define dlwlo=dlwrfsfc'
        endif
        if (hh=1)
            'define dlwnx=dlwrfsfc(t='t+2')'
            'define dlwlo=dlwrfsfc/3*2+dlwnx/3'
        endif
        if (hh=2)
            'define dlwnx=dlwrfsfc(t='t+1')'
            'define dlwlo=dlwrfsfc/3+dlwnx/3*2'
        endif
    else
      'define dlwlo=ave(dlwrfsfc, t='t', t='t+step-1')'
    endif
...
*   Simple interpolation for shortwave and wind speed
    'define dswlo=ave(dswrfsfc, t='t', t='t+step-1')'
    'define dswhi=re(nfill(dswlo,lat,'buff'), 'rehi', bl)'
    
    'define wsulo=ave(ugrd10m, t='t', t='t+step-1')'
    'define wsuhi=re(nfill(wsulo,lat,'buff'), 'rehi', bl)'
    'define wsvlo=ave(vgrd10m, t='t', t='t+step-1')'
