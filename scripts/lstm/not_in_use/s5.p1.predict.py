import sys
sys.path.append('./') ### path for monflowpred
import torch
import random
import os
import numpy as np
...
###### Read config file
#config = utils.read_config('./main.config.test.23')
#config = utils.read_config('./main.config.test.24.all.vars')
#config = utils.read_config('./main.config.test.23.all.vars')

config_f = sys.argv[1]
config = utils.read_config(config_f)
...
for ni in range(n_stn):
    ...
    tmpc3 = 1.
    flow_rec.loc[dict(id=ni)] = \
            tmpc1/(tmpc2*tmpc3) * flow_rec.sel(id=ni) / (areai*pavgi)
...
t_valid = list(config['TEST_PARA']['Tpredc'].split(','))
...
saveEpoch   =  int(config['HYPER_PARA']['EPOCHsave'])
...
for ni in range(n_stn):
    ...
    pavgi = df_stc_ori['p_mean'].iloc[ni]
    flow_rec.loc[dict(id=ni)] = \
            flow_rec.sel(id=ni) * (areai) * (tmpc2*tmpc3/tmpc1) * pavgi
...
for bi in range( int(config['HYPER_PARA']['batch_size']) ):
    ...
    predbi.to_csv(output_dir+'/'+str(bi+1)+'.predicted-flow.'+\
