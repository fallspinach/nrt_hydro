...
import sys, os, pytz, time, subprocess, csv, json
...
whmoni_path = f'{config["base_dir"]}/{modelid}/{domain}/nrt/output/1km_daily'
whrean_path = f'{config["base_dir"]}/{modelid}/{domain}/retro/output/1km_daily'
whwwrf_path = f'{config["base_dir"]}/wrf_hydro/{domain}/fcst/wwrf/output/41'
whespw_path = f'{config["base_dir"]}/wrf_hydro/basins24/fcst/esp_wwrf/output'
...
def main(argv):
    ...
    print(f'{whmoni_path}/202?/202?????.CHRTOUT_DOMAIN1.daily')
    last_whrean  = find_last_time(f'{whrean_path}/202?/202???.LDASOUT_DOMAIN1.daily', '%Y%m.LDASOUT_DOMAIN1.daily') + relativedelta(months=1) - timedelta(hours=1)
    last_whmoni  = find_last_time(f'{whmoni_path}/202?/202?????.LDASOUT_DOMAIN1.daily', '%Y%m%d.LDASOUT_DOMAIN1.daily') + timedelta(hours=23)
    ...
    # write json status file for cnrfc
    cnrfc_data =  {}
    for i in [0, 1, 2, 3, 4, 6, 7, 8]:
        cnrfc_data[datastreams[i]] = dataend[i].isoformat()
    cnrfc_data[datastreams[10]+' Start'] = datastart[10].isoformat()
    cnrfc_data[datastreams[10]+' End']   = dataend[10].isoformat()

    # Open the file in write mode ("w") and write the JSON data
    fnjson = f'{config["base_dir"]}/{modelid}/cnrfc/nrt/output/status.json'
    with open(fnjson, "w") as file:
        json.dump(cnrfc_data, file, indent=4)
