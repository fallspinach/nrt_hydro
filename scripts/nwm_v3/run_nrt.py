...
def run_wrf_hydro(domain, t1, t2, xland='rfcs', dep=-1):
    ...
    workdir = f'{config["base_dir"]}/{modelid}/{domain}/nrt/run'
    ...
    for ftpl in glob('../../../shared/nrt/*.tpl'):
        ...
        if domain=='conus' and f==frun:
            ...
            target = f'{config["base_dir"]}/{modelid}/{domain}/domain/wrfinput_conus.nc'
...
def main(argv):
    ...
    for freq in ['hourly', 'daily', 'monthly']:
        ...
        os.makedirs(f'{workdir}/../output/1km_{freq}/{t2:%Y}', exist_ok=True)
