...
def main(argv):
    ...
    for y in range(y1, y2+1):
        ...
        cmd = f'sbatch -d afterok:{jid1} -t 04:00:00 --nodes=1 -p {part_shared} --ntasks-per-node={nc_shared:d} --mem={mem_shared}G -J mf{t1:%Y%m} --wrap="{cmd1} {domain} {t1:%Y%m} {t2:%Y%m} retro" -o {workdir}/log/mergefixtime_{t1:%Y%m}_{t2:%Y%m}.txt'
