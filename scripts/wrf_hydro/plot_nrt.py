...
def main(argv):
    ...
    # read lat/lon
    fndom = f'{nrtdir}/../domain/wrfinput_{domain}.nc'
    ...
    for t in alltimes[rank::size]:
        ...
        # SMTOT and SWE
        fn = f'{nrtdir}/output/1km_daily/{t:%Y%m}.LDASOUT_DOMAIN1'
        f = nc.Dataset(fn, 'r')
        ...
        if monthly_flag:
            fn = f'{nrtdir}/forcing/1km_monthly/{t:%Y%m}.LDASIN_DOMAIN1.monthly'
            f = nc.Dataset(fn, 'r')
