...
function createLayerOption(map, layerId, label, initialVisibility, legendSymbol, legendColor) {
  ...
  checkbox.addEventListener('change', () => {
    ...
    // turn on/off colormap accordingly
    if (layerId=='nwmrivers') {
      document.getElementById('legend-wrapper').style.display = isVisible ? 'block' : 'none';
      document.getElementById('datepicker-wrapper').style.display = isVisible ? 'block' : 'none';
    }
    ...
    const hash = `${center.lat.toFixed(5)},${center.lng.toFixed(5)},${zoom},${bearing},${pitch},${style},${projection},${terrain}!${layers1}`;
