...
export function loadLayerNwmRivers(map, id, initialVisibility=false, legendColor='darkgray') {
  ...
  map.on('styledata', () => {
    ...
    if (!map.getSource(layerName+'_source')) {
      map.addSource(layerName+'_source', {
        type: 'vector',
        url: 'pmtiles://https://cw3e.ucsd.edu/wrf_hydro/conus/pmtiles/nwm_reaches_conus.pmtiles',
  ...
  const colorStops = [
    { threshold: 95, color: "magenta" },
    { threshold: 90, color: "darkmagenta" },
    { threshold: 80, color: "darkblue" },
    { threshold: 50, color: "darkcyan" },
    { threshold: 20, color: "darkgreen" },
    { threshold: 10, color: "olive" },
    { threshold: 5, color: "goldenrod" },
    { threshold: null, color: "sienna" }
  ];
  // draw colormap
