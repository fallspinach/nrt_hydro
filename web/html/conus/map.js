...
import { loadLayerGsha } from './load-layer-gsha.js';
import { loadLayerGeodar } from './load-layer-geodar.js';
...
export async function setupMap(containerID='map', coordsID='mouse-coords') {
  ...
  const visibleLayers = [
    ...
    { id: 'dwrofficial', name: 'Official Points',       load: loadLayerDwrPoints,   initialVisibility: true,  legendSymbol: '&#9679;',  legendColor: 'indigo' },
    { id: 'dwrreservoir',name: 'Reservoir Points',      load: loadLayerDwrPoints,   initialVisibility: true,  legendSymbol: '&#9679;',  legendColor: 'magenta' },
    ...
    { id: 'gsha',        name: 'GSHA (v1.1)',           load: loadLayerGsha,        initialVisibility: false, legendSymbol: '&#9679;',  legendColor: 'darkorange' },
    { id: 'geodar',      name: 'GeoDAR (v1.1)',         load: loadLayerGeodar,      initialVisibility: false, legendSymbol: '&Delta;',  legendColor: 'black' }
  ];
  ...
  function updateURLHash() {
    ...
    const hash = `${center.lat.toFixed(5)},${center.lng.toFixed(5)},${zoom},${bearing},${pitch},${currentStyle},${currentProjection},${terrain}!${layers}`;
  ...
  function loadMapStateFromHash() {
    ...
    const [viewPart, layerPart] = hash.split('!');
    ...
    if (viewParams.length>=7) {
      ...
      map.on('style.load', () => {
        ...
        layerStates.forEach(({ id, visible }) => {
          ...
          // turn on/off colormap accordingly
          if (id=='nwmrivers') {
            document.getElementById('legend-wrapper').style.display = visible ? 'block' : 'none';
            document.getElementById('datepicker-wrapper').style.display = visible ? 'block' : 'none';
          }
        });
      });
      setupLayerControl(map, visibleLayers);
